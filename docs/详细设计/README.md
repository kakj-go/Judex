...
| 存储 | PostgreSQL 保存平台事实；SQLite 承载独立执行期结构化上下文、副本与索引；文件进对象存储 | Q059、Q060、Q070 |
...
| [11 工作目录与动态上下文](./11-工作目录与动态上下文.md) | SQLite 上下文、项目共享只读挂载、派生发布、提示词组装与恢复 |

## 4. 总体边界

```mermaid
flowchart LR
    W[员工 Web] --> S[Judex Go / Gin 业务单体]
    C[员工主动调用本地 AI / CLI] --> S
    S --> P[PostgreSQL]
    S --> O[S3 对象存储]
    S --> B[OpenSandbox 控制面]
    B --> R[动态沙箱中的 Go Agent Runner]
    R -->|受限工具调用 / 执行事件 / 草稿| S
```
