...
| Agent Runner | 同集群容器沙箱中按需启动，使用指定镜像 / 程序 | 执行组件 |
...
设计安装路径：检查集群前提 → 安装 / 验证基础资源 → 等待数据库和 S3 服务可用 → 执行版本化迁移及 Bucket 初始化 → 启动业务 API 与 Runner 配置 → 冒烟验证。
...
沙箱默认无业务数据库直连权限，服务账号与 Server 分离；使用项目共享资料只读挂载、独立工作区和执行期 SQLite。限制资源、网络出口及原件写权限；不将整个平台的项目资料预挂到所有沙箱。
...
预热池使用预先配置的卷，不能假定分配时可追加任意项目挂载。首期建议验证按项目创建沙箱；预热复用作为后续优化，不能破坏项目隔离。沙箱回收不删除项目共享数据；SQLite 使用执行实例本地文件系统，不放入项目 S3 / FUSE 共享区。
