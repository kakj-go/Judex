...
同一次提交的文字与文件包有共同 submission_id；完成上传与材料登记后发布材料可用事件。Agent 的工作目录只在明确输出 / 主动提交时发布新共享草稿，临时写入不自动广播。提交接口保留来源材料版本，不能写回覆盖原件。

## 7. 内部 Runner 接口

Runner 仅取得对应 run_id 的输入、工具调用许可与事件提交能力。服务端核验该运行仍有效、绑定和访问范围未撤回，不能只凭内部网络地址信任请求。

回传事件以 run_id + seq 或 tool_call_id 去重。晚到结果保存实际来源和状态，若运行已取消，不再自动应用为有效下一步；正式业务变更仍经独立命令与人的决定。
